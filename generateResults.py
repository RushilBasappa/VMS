...
def generateStatistics_Plot1():
    ...
    for i, pageSize in enumerate(pageSizeList):
        if pageSize == PAGE_SIZE:
            ...
            processInfo = ['./a.out', PATH_TO_PROCESS_LIST,
                           PATH_TO_PROCESS_TRACE, REPLACEMENT, PAGING, pageSize, '0']
...
def generateStatistics_Plot2():
    ...
    simCombinations = [['DEMAND', 'FIFO'], ['DEMAND', 'LRU'], [
        'DEMAND', 'CLOCK'], ['PRE', 'FIFO'], ['PRE', 'LRU'], ['PRE', 'CLOCK']]
    ...
    for i, combination in enumerate(simCombinations):
        if combination[0] == PAGING and combination[1] == REPLACEMENT:
            ...
            processInfo = ['./a.out', PATH_TO_PROCESS_LIST, PATH_TO_PROCESS_TRACE,
                           combination[1], combination[0], PAGE_SIZE, '0']
    ...
    combinationList = [combination[0] + ' + ' + combination[1]
                       for combination in simCombinations]
...
def createPlot1():
    ...
    bars[pageIndex].set_color('crimson')
    ...
    for i in ax.patches:
        plt.text(i.get_width()+0.2, i.get_y()+0.45,
...
def createPlot2():
    ...
    bars = plt.barh(combinationList, pageFaultList,
                    align='center', color="darkorange")
    bars[mainIndex].set_color('crimson')
    ax.spines['left'].set_color('black')
    ax.spines['bottom'].set_color('black')
    ax.xaxis.set_tick_params(pad=5)
    ax.yaxis.set_tick_params(pad=10)
    ...
    plt.ylabel('Combinations of paging and replacement policies')
...
def executeMainRequest():
    ...
    processInfo = ['./a.out', PATH_TO_PROCESS_LIST,
                   PATH_TO_PROCESS_TRACE, REPLACEMENT, PAGING, PAGE_SIZE, '1']
...
def printData():
    ...
    print(processCount, memoryRequestCount, PAGING,
          REPLACEMENT, PAGE_SIZE, pageFaults, end='')
...
if __name__ == '__main__':
    ...
    ProgressWin = Tk()
    ProgressWin.title('Virtual memory simulator- Processing')
    ProgressWin.config(bg='grey')
    ...
    ProgressWin.geometry("{}x{}+{}+{}".format(window_width,
                         window_height, x_cordinate, y_cordinate))

    # Creating a main frame inside the root window
    main_frame = Frame(ProgressWin, relief=GROOVE, bg='gray')
    main_frame.place(x=10, y=10)  # Placing the frame at (10, 10)

    # Creating sub- frames
    frame1 = Frame(main_frame, padx=3, pady=3, bg='gray')
    ...
    label = Label(master=frame1, text="Running simulations!!...",
                  font="Verdana 15 bold", bg='gray')
    label.pack()  # Put the label into the window

    # Progress bar widget
    progress = Progressbar(frame2, orient=HORIZONTAL,
