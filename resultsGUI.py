...
def displayOutputInCanvas():
    ...
    frame1 = Frame(master=master_frame, width=1530, pady=10)
    frame2 = Frame(master=master_frame, width=1530, pady=20)
    frame3 = Frame(master=master_frame, width=1530, pady=5)
    frame4 = Frame(master=master_frame, width=1530, pady=20)
    frame5 = Frame(master=master_frame, width=1530, pady=10)
    frame6 = Frame(master=master_frame, width=1530, pady=10)
    frame7 = Frame(master=master_frame, width=1530, pady=10)
    frame8 = Frame(master=master_frame, width=1530, pady=10)
    frame9 = Frame(master=master_frame, width=1530, pady=10)
    ...
    title = Label(master=frame1, text='VIRTUAL MEMORY SIMULATOR',
                  font='Script 35')
    title.pack()

    subtitle1 = Label(master=frame2, text='Results',
                      font='Script 25')
    ...
    for i in range(height):  # Rows
        for j in range(width):  # Columns
            ...
            b = Entry(frame3, fg=fgcolor, font='Script 20', bg='black', highlightthickness=0, highlightbackground="black",
                      borderwidth=0, width=40, disabledbackground='black', disabledforeground=fgcolor)
            b.grid(row=i, column=j)
            b.insert(END, data[i][j])
            b['state'] = DISABLED

    subtitle2 = Label(master=frame4, text='Statistics',
                      font='Script 25')
    subtitle2.pack(side=LEFT, padx=10)

    plottitle1 = Label(master=frame5, text='Plot 1: Number of pagefaults VS Page size',
                       font='Script 20')
    ...
    plottitle2 = Label(master=frame7, text='Plot 2: Number of pagefaults for different combinations of paging and replacement methods',
                       font='Script 20')
...
if __name__ == '__main__':
    ...
    master_frame = Frame(canvas, padx=10, pady=10)
