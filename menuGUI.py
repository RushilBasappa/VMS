...
frame1 = Frame(master = root, height = 100, width = 1000, pady = 10)
frame2 = Frame(master = root, height = 50, width = 1000, pady = 20)
frame3 = Frame(master = root, height = 50, width = 1000, pady = 20)
frame4 = Frame(master = root, height = 50, width = 1000, pady = 20)
frame5 = Frame(master = root, height = 50, width = 1000, pady = 20)
frame6 = Frame(master = root, height = 50, width = 1000, pady = 20)
frame7 = Frame(master = root, height = 50, width = 1000, pady = 20)
...
title = Label(master=frame1,text='VIRTUAL MEMORY SIMULATOR',font='Script 35')
title.pack()

plist_path_label = Label(master=frame2,text='Process list :', width=16, font='Script 20')
plist_path_label.pack(side=tk.LEFT,padx=10)

plist_path_var=StringVar()
plist_path=''
plist_path_entry = Entry(master=frame2,textvariable=plist_path_var,width=40, font='Script 20')
...
ptrace_path_label = Label(master=frame3,text='Process trace: ', width=16, font='Script 20')
ptrace_path_label.pack(side=tk.LEFT,padx=10)

ptrace_path_var=StringVar()
ptrace_path=''
ptrace_path_entry = Entry(master=frame3, textvariable=ptrace_path_var, width=40, font='Script 20')
...
label = Label(master=frame4,text='Fetch Policy:',font='Script 20', width=16)
label.pack(side=tk.LEFT,padx=10)

c1 = StringVar()
combobox1 = ttk.Combobox(master=frame4,textvariable=c1,font='Script 20', width=20, state='readonly')
combobox1['values']=('DEMAND','PRE')
combobox1.current()
combobox1.pack(side=tk.LEFT,padx=10)

label = Label(master=frame5,text='Replacement Policy:',font='Script 20', width=16)
label.pack(side=tk.LEFT,padx=10)

c2 = StringVar()
combobox2 = ttk.Combobox(master=frame5, textvariable=c2, font='Script 20', width=20, state='readonly')
combobox2['values']=('FIFO','LRU','CLOCK')
combobox2.current()
combobox2.pack(side=tk.LEFT,padx=10)

label = Label(master=frame6,text='Page Size:',font='Script 20', width=16)
label.pack(side=tk.LEFT,padx=10)

c3 = IntVar()
combobox3 = ttk.Combobox(master=frame6,textvariable=c3,font='Script 20', width=20, state='readonly')
...
btn = Button(master=frame7,text='Set Default',font='Script 20',command=setDefault)
btn.pack(side=tk.LEFT,padx=10)

btn = Button(master=frame7,text='Submit',font='Script 20',command=submit)
btn.pack(side=tk.LEFT,padx=10)
